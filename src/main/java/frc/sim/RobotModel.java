...
public class RobotModel {
  ...
  // Mechanical rotation driven by motor with gear reduction for simulation purposes.
  // Works in conjunction with RotationSubsystem
  RotationModel simRotation;
  ...
  public RobotModel(Robot robot) {
    ...
    simRotation = new RotationModel(robot.getRobotContainer().getRotationSubsystem());
  ...
  public void update() {
    ...
    simRotation.updateSim();

    // Simulate battery voltage drop based on total simulated current
    double rotationCurrent = Math.abs(simRotation.getSimCurrent());

    double[] simCurrents = {rotationCurrent};
    ...
    simpdp.setCurrent(7, rotationCurrent);
